...
LL %>%
  ...
  rowwise() %>% mutate(Cases = Cases/c_max, Severe_cases=Severe_cases/sc_max) %>%
  gather("variable","value",12:17) %>%
  #mutate(x_Temporary_Larval_Habitat = paste(x_Temporary_Larval_Habitat)) %>%
...
LL %>%
  ...
  filter(!bites_flag)  %>%
  ungroup() %>%
  summarize("Temp_Rain" = range(scale_temp_rain),
            "Water_Veg"=range(scale_water_veg),
            "Constant"=range(scale_constant),
            "x_Temporary_Larval_Habitat"=range(x_Temporary_Larval_Habitat))
